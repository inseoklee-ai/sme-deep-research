...
API 키는 저장소에 넣지 않습니다. `C:\Users\lis29\projects\keys.env` 또는 프로젝트 폴더의 `.env` 에 `OPENAI_API_KEY=sk-...` 한 줄을 두면 읽습니다(두 파일 모두 `.gitignore` 에 있음). 다른 경로를 쓰려면 환경 변수 `KEYS_ENV` 로 지정합니다.
...
- **새로 질문하기** — 예시 질문 11건 중 고르거나 직접 쓰고, 왼쪽에서 스위치(역할·배정·구역·재위임)를 끄고 켜며 돌려 봅니다. 「혼자 하는 대조군과 비교」를 켜면 팀이 실제로 읽은 건수만큼 혼자 읽은 보고서를 나란히 보여 줍니다. 키가 파일에 없으면 왼쪽에 입력합니다(저장하지 않음).
